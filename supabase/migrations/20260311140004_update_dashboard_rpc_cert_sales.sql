-- Update Dashboard RPC to include certificate_sales in revenue/profit calculations
...
BEGIN
  ...
  IF start_date IS NOT NULL THEN
    ...
    IF end_date IS NOT NULL THEN
      chart_end_date := end_date - interval '1 day';
    ELSE
      chart_end_date := now();
    END IF;
  ELSE
    ...
    IF chart_grouping = 'day' AND extract(day FROM chart_end_date - chart_start_date) > 365 THEN
       chart_start_date := chart_end_date - interval '365 days';
    END IF;
  END IF;
  ...
  sold_base_items AS (
    ...
    WHERE organization_id = org_id
      ...
      AND (start_date IS NULL OR (sale_date IS NOT NULL AND sale_date >= start_date))
  ...
  sold_cert_items AS (
    ...
    WHERE ic.organization_id = org_id
      ...
      AND (start_date IS NULL OR (i.sale_date IS NOT NULL AND i.sale_date >= start_date))
      AND (end_date IS NULL OR (i.sale_date IS NOT NULL AND i.sale_date < end_date))
  ),
  -- NEW: Include standalone certificate sales
  cert_sales AS (
    SELECT
      sale_price_eur,
      cost_eur as purchase_price_eur,
      0 as fees,
      0 as shipping,
      sale_channel,
      COALESCE(provider_name, 'Zertifikat') as brand,
  ...
  filtered_sold_items AS (
    SELECT * FROM sold_base_items
    UNION ALL
    SELECT * FROM sold_cert_items
    UNION ALL
    SELECT * FROM cert_sales
  ),
  ...
  stock_items AS (
    SELECT * FROM stock_base_items
    UNION ALL
    SELECT * FROM stock_cert_items
  ),
  ...
  period_recurring AS (
     SELECT 
       p.period_start,
       SUM(r.amount_eur) as amount
     FROM time_periods p
     JOIN recurring_expenses r ON r.start_date <= (p.period_start + chart_interval - interval '1 day')
     WHERE 
       (chart_grouping = 'month' AND 
         (
           (r.recurring_interval = 'yearly' AND EXTRACT(MONTH FROM r.start_date) = EXTRACT(MONTH FROM p.period_start)) OR
           (r.recurring_interval = 'semi_annually' AND MOD((CAST(EXTRACT(YEAR FROM p.period_start) - EXTRACT(YEAR FROM r.start_date) AS INTEGER) * 12 + CAST(EXTRACT(MONTH FROM p.period_start) - EXTRACT(MONTH FROM r.start_date) AS INTEGER)), 6) = 0) OR
           (r.recurring_interval = 'quarterly' AND MOD((CAST(EXTRACT(YEAR FROM p.period_start) - EXTRACT(YEAR FROM r.start_date) AS INTEGER) * 12 + CAST(EXTRACT(MONTH FROM p.period_start) - EXTRACT(MONTH FROM r.start_date) AS INTEGER)), 3) = 0) OR
           (r.recurring_interval = 'monthly')
         )
       ) OR
       (chart_grouping = 'week' AND 
         (
           (r.recurring_interval = 'monthly' AND EXTRACT(DAY FROM r.start_date) BETWEEN EXTRACT(DAY FROM p.period_start) AND EXTRACT(DAY FROM (p.period_start + interval '6 days')))
         )
       ) OR
       (chart_grouping = 'day' AND 
         (
           (r.recurring_interval = 'monthly' AND EXTRACT(DAY FROM r.start_date) = EXTRACT(DAY FROM p.period_start)) OR
           (r.recurring_interval = 'yearly' AND EXTRACT(MONTH FROM r.start_date) = EXTRACT(MONTH FROM p.period_start) AND EXTRACT(DAY FROM r.start_date) = EXTRACT(DAY FROM p.period_start))
         )
       ) OR
       (chart_grouping NOT IN ('month', 'week', 'day'))
