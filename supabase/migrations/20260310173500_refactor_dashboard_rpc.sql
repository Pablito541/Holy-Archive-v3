...
BEGIN
  ...
  ELSE
    ...
    -- Limit distance to prevent generate_series explosion on 'all' with 'day'
  ...
  -- Create data series dynamically limits to 1000 to prevent DOS
  time_periods AS (
    SELECT period_start 
  ...
  -- Instead of CROSS JOIN, only JOIN if applicable based on interval logic to create sparse periods
  period_recurring AS (
     SELECT 
       p.period_start,
