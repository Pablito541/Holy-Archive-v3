...
-- 2. Update RLS policies to exclude deleted items for normal views, but since they can't be updated through SECURITY DEFINER as easily we will redefine the select/update policies.

-- Items policy
DROP POLICY IF EXISTS "Organization Members Access" ON public.items;
CREATE POLICY "Organization Members Access" ON public.items FOR ALL USING (
  deleted_at IS NULL AND
  auth.uid() IN (SELECT user_id FROM organization_members WHERE organization_id = items.organization_id)
);

-- Update public item policy to also respect deleted_at
...
-- Expenses policy
DROP POLICY IF EXISTS "Organization Members Access" ON public.expenses;
CREATE POLICY "Organization Members Access" ON public.expenses FOR ALL USING (
  deleted_at IS NULL AND
  auth.uid() IN (SELECT user_id FROM organization_members WHERE organization_id = expenses.organization_id)
);

-- 3. Update the Dashboard RPC to fully ignore deleted items, since SECURITY DEFINER bypasses RLS
...
BEGIN
  ...
  ELSE
    ...
    chart_start_date := COALESCE(
      (SELECT MIN(sale_date) FROM items WHERE organization_id = org_id AND status = 'sold' AND deleted_at IS NULL),
      now()
    );
    chart_end_date := now();
  END IF;
  ...
  stock_items AS (
    ...
    WHERE organization_id = org_id AND (status = 'in_stock' OR status = 'reserved') AND deleted_at IS NULL
    UNION ALL
    SELECT ic.cost_eur as purchase_price_eur FROM item_certificates ic
    JOIN items i ON ic.item_id = i.id
    WHERE ic.organization_id = org_id AND (i.status = 'in_stock' OR i.status = 'reserved') AND i.deleted_at IS NULL
  ),
  ...
  recurring_expenses AS (
    ...
    FROM expenses
    WHERE organization_id = org_id AND is_recurring = true AND deleted_at IS NULL
  ),
